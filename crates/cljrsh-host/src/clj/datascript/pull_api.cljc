(ns ^:no-doc datascript.pull-api
  (:require
    ...
    [datascript.db :as db #?@(:cljrsh [:refer [DB]] :cljs [:refer [DB]])]
...
(defn- first-seq [#?(:cljrsh xs :clj ^ISeq xs :cljs ^seq xs)]
  (if (nil? xs)
    nil
    #?(:cljrsh (first xs) :clj (.first xs) :cljs (-first xs))))

(defn- next-seq [#?(:cljrsh xs :clj ^ISeq xs :cljs ^seq xs)]
  (if (nil? xs)
    nil
    #?(:cljrsh (next xs) :clj (.next xs) :cljs (-next xs))))

(defn- conj-seq [#?(:cljrsh xs :clj ^ISeq xs :cljs ^seq xs) x]
  (if (nil? xs)
    (list x)
    #?(:cljrsh (cons x xs) :clj (.cons xs x) :cljs (-conj xs x))))
  ...
  (-run [this context]
    (loop [acc    acc
      ...
      (util/cond+
        ...
        ;; default
        (and datom-ahead? (some? (#?(:cljrsh :default :clj .-default :cljs :default) attr)))
        (recur (assoc! acc (.-as attr) (#?(:cljrsh :default :clj .-default :cljs :default) attr)) (first-seq attrs) (next-seq attrs) datoms)
  ...
  (-run [this context]
    (loop [acc   acc
      ...
      (util/cond+
        ...
        (and (empty? datoms) (some? (#?(:cljrsh :default :clj .-default :cljs :default) attr)))
        (recur (assoc! acc (.-as attr) (#?(:cljrsh :default :clj .-default :cljs :default) attr)) (first-seq attrs) (next-seq attrs))
