...
#?(:clj
   ;; cljrsh: the parameter was named `cond`; a local shadowing a core macro
   ;; name breaks qualified macro resolution here, so it is renamed.
   (defn- if+-rewrite-cond-impl [clauses]
     (clojure.core/cond
       (empty? clauses)
       true
       ...
       (and
         (= :let (first clauses))
         (empty? (second clauses)))
       (if+-rewrite-cond-impl (nnext clauses))
    
       (= :let (first clauses))
       (let [[var val & rest] (second clauses)
             sym                (gensym)]
       ...
       (list 'and
         (first clauses)
         (if+-rewrite-cond-impl (next clauses))))))
