(ns ^:no-doc datascript.query
  (:require
    ...
    [datascript.parser :as dp #?@(:cljrsh [:refer [BindColl BindIgnore BindScalar BindTuple Constant
...
;; cljrsh: no Eduction type; a lazy transducer application is equivalent
;; for these read-only pipelines.
#?(:cljrsh (defn- ->Eduction [xform coll] (sequence xform coll)))
#?(:cljrsh (defn- aclone [a] (da/aclone a)))
;; cljrsh: one accessor for every tuple representation a relation can hold:
;; object arrays (joins), vectors (plain collections), and Datom records
;; (db index lookups, which on the JVM answer nth/valAt through interfaces).
#?(:cljrsh
   (defn- tuple-get [t i]
     (cond
       (da/array? t)
       ...
       ;; db index relations key attrs by datom field name ("e" "a" "v");
       ;; collection relations key by int position.
       (db/datom? t)
       (if (int? i)
         (case (int i)
           0 (:e t) 1 (:a t) 2 (:v t) 3 (db/datom-tx t) 4 (db/datom-added t))
         (case (str i)
           "e" (:e t) "a" (:a t) "v" (:v t)
           "tx" (db/datom-tx t) "added" (db/datom-added t)))
                    ...
                    (fn [acc tuple-b]
                      (let [tuple' (da/make-array tlen)]
                        (doseq [[idx-b idx-a] idxb->idxa]
                          (aset tuple' idx-a #?(:cljrsh (tuple-get tuple-b idx-b) :cljs (da/aget tuple-b idx-b) :clj (get tuple-b idx-b))))
                        (conj! acc tuple')))
      ...
      (if (int? idx)
        (let [idx (int idx)]
          (fn contained-int-getter-fn [tuple]
            (let [eid #?(:cljrsh (if (da/array? tuple)
                                    (da/aget tuple idx)
                                    (nth tuple idx))
        ...
        (fn contained-getter-fn [tuple]
          (let [eid #?(:cljrsh (tuple-get tuple idx)
                       :cljs (da/aget tuple idx)
...
(defn tuple-key-fn
  ...
  (let [n (count common-attrs)]
    (if (== n 1)
      ...
      (let [^objects getters-arr #?(:cljrsh (da/into-array common-attrs)
        ...
        (loop [i 0]
          (if (< i n)
            ...
            #?(:cljrsh
               (fn [tuple]
                 (mapv (fn [g] (g tuple)) (vec getters-arr)))
               :clj
               (fn [tuple]
      ...
      (when-some [tuple (first (:tuples rel))]
        (when (nil? (fnext (:tuples rel)))
          (let [idx (get (:attrs rel) pattern-el)]
            #?(:cljrsh (tuple-get tuple idx) :cljs (da/aget tuple idx) :clj (get tuple idx))))))))
...
(defn- context-resolve-val [context sym]
  (when-some [rel (rel-with-attr context sym)]
    (when-some [tuple (first (:tuples rel))]
      #?(:cljrsh (tuple-get tuple ((:attrs rel) sym)) :cljs (da/aget tuple ((:attrs rel) sym)) :clj (get tuple ((:attrs rel) sym))))))
        ...
        (let [args (da/aclone static-args)]
          (dotimes [i len]
            (when-some [tuple-idx (aget tuples-args i)]
              (let [v #?(:cljrsh (tuple-get tuple tuple-idx)
                         :cljs (da/aget tuple tuple-idx)
...
(defn check-bound [bound vars form]
  ;; cljrsh: vars arrives as a lazy seq; the native subset? wants a set.
  (when-not (set/subset? (set vars) bound)
            ...
            (map
              (fn [t2]
                (let [res (aclone t1)]
                  #?(:cljrsh
                     (dotimes [i len]
                       (when-some [idx (da/aget copy-map i)]
