...
#?(:cljrsh nil :clj (set! *warn-on-reflection* true))
...
(defn #?@(:clj  [^Boolean seqable?]  
  ...
  (and (not (string? x))
    #?(:cljs (or (cljs.core/seqable? x)
               (arrays/array? x))
       :cljrsh (or (seq? x) (nil? x) (coll? x) (arrays/array? x))
       :clj  (or (seq? x)
...
(defn class-identical?
  ...
  [x y]
  #?(:cljrsh (= (type x) (type y))
     :clj  (identical? (class x) (class y))
     :cljs (identical? (type x) (type y))))

#?(:cljrsh
   (defn class-name [x] (if (nil? x) x (str (type x))))
   :clj
   (defn class-name
     {:inline
      (fn [x]
...
(defn class-compare
  ^long [x y]
  #?(:cljrsh (compare (class-name x) (class-name y))
...
(defn+ ^number value-compare [x y]
  (try
    (cond
      ...
      #?@(:cljrsh [(number? x) (compare x y)])
      #?@(:clj  [(instance? Number x)       (clojure.lang.Numbers/compare x y)])
      #?@(:cljrsh [(class-identical? x y) (compare x y)]
    ...
    (catch #?(:cljrsh Exception :clj ClassCastException :cljs js/Error) e
...
;; cljrsh: the shimmed sorted sets have no transient mode; identity is
;; correct, just without the bulk-load speedup.
