...
(ns ^:no-doc datalevin.query-optimizer
  ...
  (:require
   [clojure.set :as set]
   #?@(:cljrsh [] :clj [[clojure.core.reducers :as rd]])
   [clojure.walk :as w]
   ...
   [datalevin.join :as j]
   #?@(:cljrsh [[datascript.lru :as lru]] :clj [[datalevin.lmdb :as l]])
   [datalevin.parser :as dp
    #?@(:cljrsh [:refer [And BindColl BindIgnore BindScalar BindTuple
                         Constant DefaultSrc Function Or Variable Pattern
                         Predicate Not RuleExpr]] :clj [])]
...
;; by append path); the plan cache is datascript's LRU in a volatile;
;; writing? is false (no datalevin write transactions on cljrs).
#?(:cljrsh
   (do
     (defrecord DPK [members path ordered?])
     (defprotocol IDPKey
       (contains [k node])
       (append [k node])
       (members [k])
     ...
     (extend-type DPK
       IDPKey
       (contains [k node]
         (not (zero? (bit-and (long (:members k))
                              (bit-shift-left 1 (long node))))))
       (append [k node]
         (let [m (bit-or (long (:members k))
                         (bit-shift-left 1 (long node)))]
           (if (:ordered? k)
             (->DPK m (conj (:path k) (long node)) true)
             (->DPK m nil false))))
       (members [k] (:members k))
       (isOrdered [k] (:ordered? k)))
     (defn- dpkey-ordered [node]
       (->DPK (bit-shift-left 1 (long node)) [(long node)] true))
     (defn- dpkey-canonical [m] (->DPK (long m) nil false))))

(def ^:dynamic *plan-cache*
  #?(:cljrsh (volatile! (lru/cache c/query-result-cache-size))
     :clj (LRUCache. c/query-result-cache-size)))
...
(defn- count-init-follows
  ...
  (let [store (.-store db)]
    #?(:cljrsh
       (reduce + 0 (map #(av-size store attr (aget ^objects % index))
    ...
    (reduce (fn [key e]
              (let [node (int (node-ids e))]
                (if key
                  ...
                  (#?(:cljrsh dpkey-ordered :clj DPKey/ordered) node))))
            nil entities)
    (#?(:cljrsh dpkey-canonical :clj DPKey/canonical)
...
(defn- dp-initial-plans
  ...
  (persistent!
    (reduce-kv (fn [plans [e] plan]
                 (assoc! plans (#?(:cljrsh dpkey-ordered :clj DPKey/ordered) (int (node-ids e))) plan))
...
(defn- shrink-space
  ...
  (persistent!
    (reduce-kv
      (fn [m k ps]
        (assoc! m (if node-ids (#?(:cljrsh dpkey-canonical :clj DPKey/canonical) (long k)) k)
        ...
        (fn [c src nodes]
          (let [^DB db        (sources src)
                ...
                k             [(.-store db) nodes required-vars
                               projected-vars]]
            (if-let [cached #?(:cljrsh (clojure.core/get @(plan-cache) k)
                               :clj (.get ^LRUCache (plan-cache) k))]
                ...
                (if (some #(zero? (long (:mcount %))) (vals nodes))
                  ...
                  (let [{:keys [plans deferred attribute-group-planning]}
                    ...
                    (if (some #(some nil? %) plans)
                      (reduced (assoc c :result-set #{}))
                      (do #?(:cljrsh
                             (vswap! (plan-cache) clojure.core/assoc k
                                     {:plans (strip-result plans)
                                      :deferred deferred
                                      :attribute-group-planning
                                      attribute-group-planning})
                             :clj
                             (.put ^LRUCache (plan-cache) k
                                   {:plans (strip-result plans)
                                    :deferred deferred
                                    :attribute-group-planning
                                    attribute-group-planning}))
