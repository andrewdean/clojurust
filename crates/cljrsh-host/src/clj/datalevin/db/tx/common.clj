...
;;
;; cljrs port note: the :cljrsh arms skip the TreeSortedSet tx-cache
;; probes ((:avet db) is nil — pending-tx overlays are not ported) and
;; use the portable array test.
...
(defn multi-value?
  ...
  (and
    (is-attr? db attr :db.cardinality/many)
    (or
      #?(:cljrsh (u/array? value)
         :clj (and (some? value) (.isArray (class value))))
  ...
  (cond
    ...
    (let [[attr value] eid]
      (cond
        ...
        :else
        (or #?(:cljrsh nil
               :clj (:e (sf (.subSet ^TreeSortedSet (:avet db)
