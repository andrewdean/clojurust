...
(ns datalevin.built-ins
  ...
  (:require
   ...
   #?(:cljrsh nil :clj [datalevin.embedding :as emb])
   #?(:cljrsh nil :clj [datalevin.query.tuple :as qtuple])
   #?(:cljrsh nil :clj [datalevin.udf :as udf-reg])
   #?(:cljrsh nil :clj [datalevin.vector :as v])
   #?(:cljrsh nil :clj [datalevin.entity :as de])
   #?(:cljrsh nil :clj [datalevin.remote :as r])
...
#?(:cljrsh (def ^:no-doc like-cache (atom {}))
   :clj (def ^:no-doc like-cache (LRUCache. 256)))
#?(:cljrsh (def ^:no-doc not-like-cache (atom {}))
   :clj (def ^:no-doc not-like-cache (LRUCache. 256)))
#?(:cljrsh (def ^:no-doc like-options-cache (atom {}))
   :clj (def ^:no-doc like-options-cache (LRUCache. 256)))

#?(:cljrsh
   ;; SQL LIKE compiled to an anchored regex: % is any run, _ any char,
   ;; the escape character quotes the next character literally.
   (defn- compile-like
     [pattern escape not?]
     (let [specials (clojure.core/set ".^$*+?()[]{}|\\")
           body (loop [cs (seq pattern) acc ""]
                  (if cs
                    (let [c (first cs)]
                      (cond
                        (clojure.core/and escape (clojure.core/= c escape))
                        (let [n (second cs)]
                          (recur (nnext cs)
                                 (clojure.core/str acc (if (specials n)
                                                         (clojure.core/str "\\" n)
                                                         n))))
                        (clojure.core/= c \%) (recur (next cs) (clojure.core/str acc ".*"))
                        (clojure.core/= c \_) (recur (next cs) (clojure.core/str acc "."))
                        (specials c) (recur (next cs) (clojure.core/str acc "\\" c))
                        :else (recur (next cs) (clojure.core/str acc c))))
                    acc))
           re (re-pattern (clojure.core/str "(?s)^" body "$"))
           f  #(clojure.core/some? (re-matches re %))]
...
(defn missing?
  ...
  e.g. [(missing? $ ?e :sales)]"
  [db e a]
  #?(:cljrsh (raise "missing? needs the db layer, not yet available on cljrsh"
                    {:error :query/where})
     :clj (clojure.core/nil? (clojure.core/get (de/entity db e) a))))
...
#?(:cljrsh
(do
  (defn- storage-only [name*]
    (fn [& _]
      (raise (clojure.core/str name* " requires the storage engine, which is not available on cljrsh yet")
             {:error :query/where})))
  (def fulltext (storage-only "fulltext"))
  (def ^:no-doc fulltext-request (storage-only "fulltext-request"))
  (def ^:no-doc execute-fulltext-request (storage-only "execute-fulltext-request"))
  (def ^:no-doc fulltext-request-results (storage-only "fulltext-request-results"))
  (def fulltext-datoms (storage-only "fulltext-datoms"))
  (def ^:no-doc vec-neighbors-request (storage-only "vec-neighbors-request"))
  (def ^:no-doc embedding-neighbors-request (storage-only "embedding-neighbors-request"))
  (def ^:no-doc vector-request-results (storage-only "vector-request-results"))
  (def ^:no-doc execute-vector-request (storage-only "execute-vector-request"))
  (def vec-neighbors (storage-only "vec-neighbors"))
  (def embedding-neighbors (storage-only "embedding-neighbors"))
  (def ^:no-doc idoc-match-domain (storage-only "idoc-match-domain"))
  (def ^:no-doc prepare-idoc-match-cursor (storage-only "prepare-idoc-match-cursor"))
  (def ^:no-doc next-idoc-match-cursor-batch (storage-only "next-idoc-match-cursor-batch"))
  (def ^:no-doc close-idoc-match-cursor (storage-only "close-idoc-match-cursor"))
  (def ^:no-doc idoc-match-request (storage-only "idoc-match-request"))
  (def ^:no-doc execute-idoc-match-request (storage-only "execute-idoc-match-request"))
  (def idoc-match (storage-only "idoc-match"))
  (def idoc-get (storage-only "idoc-get")))
:clj
(do
(defn- fulltext*
...
#?(:cljrsh
   (defn udf
     ...
     [& _]
     (raise "udf requires the storage engine, which is not available on cljrsh yet"
            {:error :query/where}))
   :clj
   (defn udf
  "Resolve and invoke a runtime UDF descriptor in query context."
