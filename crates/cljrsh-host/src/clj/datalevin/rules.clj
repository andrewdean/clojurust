...
;; cljrs port note: runs near-verbatim on the runtime's mutable
;; java.util collection shims; the only :cljrsh divergence is the
;; portable fill-rule-output-and-hash below (the hash is unused —
;; portable tuple identity is the tuple's vec).
#?(:cljrsh
   (defn- fill-rule-output-and-hash
     [^objects output ^longs head-types ^ints head-idxs
      ^objects delta-tuple ^ints call-rel-idxs eav0 eav1]
     (dotimes [i (alength output)]
       (let [index (aget head-idxs i)
             value (if (zero? (aget head-types i))
                     (aget delta-tuple (aget call-rel-idxs index))
                     (if (zero? index) eav0 eav1))]
         (aset output i value)))
     0))
...
(defn- add-fast-output-with-probe!
  [^FastList acc ^HashSet seen-set seen-lookup ^objects scratch
   ...
   eav0 eav1]
  (let [h #?(:cljrsh (fill-rule-output-and-hash
                        scratch head-types head-idxs delta-tuple
                        call-rel-idxs eav0 eav1)
             :clj (ArrayUtil/fillRuleOutputAndHash
