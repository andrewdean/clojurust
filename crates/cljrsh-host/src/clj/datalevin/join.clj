...
;;
;; cljrs port note: the :cljrsh arm keeps the same public surface
;; (getter-fn, tuple-key-fn, hash-tuples, hash-join, hash-join-into,
;; subtract-rel) over persistent maps — join keys are vecs (or the
;; bare value for single-attr joins) and hash-tuples buckets are
;; always vectors of tuples, never a bare tuple. Sinks go through
;; datalevin.pipe/add-one. The :clj arm is the upstream original.
(ns ^:no-doc datalevin.join
  "Join algorithms"
  (:require
   [datalevin.relation :as r]
   [datalevin.db :as db]
   #?(:cljrsh [datalevin.pipe :as p])
...
(defn tuple-key-fn
  "Key extractor for COMMON-ATTRS: the bare value for one attr, a vec
  for several; lookup-ref attrs resolve to eids."
  [attrs common-attrs]
  (if (== 1 (count common-attrs))
    (getter-fn attrs (first common-attrs))
    (let [getters (mapv #(getter-fn attrs %) common-attrs)]
      (fn tuple-key [tuple]
        (mapv #(% tuple) getters)))))

(defn hash-tuples
  "Hash TUPLES by KEY-FN: {key [tuple ...]}."
  [key-fn tuples]
  (persistent!
    (reduce
      (fn [m t]
...
(defn- join-into*
  ...
  (let [tuples1      (:tuples rel1)
        ...
        keep-idxs2   (int-array (map attrs2 keep-attrs2))
    ...
    (when (and tuples1 tuples2)
      (if (< (count tuples1) (count tuples2))
        (let [hash (hash-tuples key-fn1 tuples1)]
          (doseq [tuple2 tuples2]
            (doseq [tuple1 (get hash (key-fn2 tuple2))]
              (emit (r/join-tuples tuple1 keep-idxs1 tuple2 keep-idxs2)))))
        (let [hash (hash-tuples key-fn2 tuples2)]
          (doseq [tuple1 tuples1]
            (doseq [tuple2 (get hash (key-fn1 tuple1))]
              (emit (r/join-tuples tuple1 keep-idxs1
                                   tuple2 keep-idxs2)))))))))
...
(defn hash-join
  ...
  (let [attrs1      (:attrs rel1)
        ...
        acc         (volatile! [])]
    (join-into* rel1 rel2 #(vswap! acc conj %))
    (r/relation! attrs @acc)))

(defn hash-join-into
  [rel1 rel2 sink]
  (join-into* rel1 rel2 #(p/add-one sink %))
  sink)

(defn subtract-rel
  [a b]
  (let [attrs    (qu/intersect-keys (:attrs a) (:attrs b))
        key-fn-b (tuple-key-fn (:attrs b) attrs)
        hash     (hash-tuples key-fn-b (:tuples b))
        key-fn-a (tuple-key-fn (:attrs a) attrs)]
    (assoc a :tuples
           (into []
                 (remove #(contains? hash (key-fn-a %)))
                 (:tuples a)))))
