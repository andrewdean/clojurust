...
(ns ^:no-doc datalevin.pull-api
  ...
  (:require
   [datalevin.pull-parser :as dpp
    #?@(:cljrsh [:refer [PullAttr PullPattern]] :clj [])]
   [datalevin.db :as db]
   [datalevin.constants :as c]
   [datalevin.datom :as dd]
   #?@(:cljrsh [] :clj [[datalevin.remote :as r]])
...
(defn parse-opts
  ...
  ([^DB db pattern {:keys [visitor]}]
   {:pattern #?(:cljrsh (dpp/parse-pattern db pattern)
                :clj (let [^LRUCache c (.-pull-patterns db)]
                       (or (.get c pattern)
...
(defn pull
  ([db pattern id opts]
   #?(:cljrsh (pull* db pattern id opts)
      :clj (let [store (.-store ^DB db)]
...
(defn pull-many
  ([db pattern id opts]
   #?(:cljrsh (pull-many* db pattern id opts)
      :clj (let [store (.-store ^DB db)]
             (if (instance? DatalogStore store)
