...
#?(:cljrsh (def +separator+ "/")
   :clj (def +separator+ java.io.File/separator))
...
#?(:cljrsh
   (defmacro defcomp
     [sym [arg1 arg2] & body]
     `(defn ~sym [~(with-meta arg1 {}) ~(with-meta arg2 {})]
        ~@body))
   :clj
   (defmacro defcomp
     [sym [arg1 arg2] & body]
...
#?(:cljrsh
   (defn combine-hashes [x y] (hash-combine x y))
   :clj
...
#?(:cljrsh
   (defn distinct-by
     [f coll]
     (peek (reduce (fn [[seen res :as acc] el]
                     (let [key (f el)]
...
#?(:cljrsh
   (defn list-add [lst item] (conj lst item))
   :clj
   (defn list-add [^FastList lst item] (.add lst item) lst))

#?(:cljrsh
   (defn map-fl [f coll] (mapv f coll))
   :clj
   (defn map-fl
     [f coll]
...
(defn long-dec ^long [^long x] (dec x))
...
(defn- mix-sampling-seed
  ...
  (let [z (bit-xor seed
                   ...
                   #?(:cljrsh (bit-or (bit-shift-left n 32)
                                      (unsigned-bit-shift-right n 32))
                      :clj (Long/rotateLeft n 32)))
...
#?(:cljrsh
   (defn reservoir-sampling
     "Random sample n of m item indices, sorted; nil when n > m. A seeded
     xorshift replaces java.util.Random; the cache is a plain atom map."
     [m n]
     (let [seed      *reservoir-sampling-seed*
           cache-key (if (some? seed) [m n seed] [m n])]
       (or (get @sample-cache cache-key)
           (let [res (cond
                       (< n m)
                       (let [state (volatile!
                                     (if (some? seed)
                                       (mix-sampling-seed (long seed) m n)
                                       (hash [m n (gensym)])))
                             next-long
                             (fn []
                               (let [s (vswap! state
                                 ...
                                 (bit-and s 0x7FFFFFFFFFFFFFFF)))]
                         (vec (sort (loop [indices (vec (range n)) i n]
                                      (if (< i m)
                                        (recur (assoc indices (mod (next-long) n) i)
                                               (+ i 1 (long (/ (Math/log (- 1.0 (/ (double (mod (next-long) 1000000)) 1000000.0)))
                                                               (Math/log (- 1.0 (double (/ n m))))))))
                                        indices)))))
                       (= n m) (vec (range n))
                       :else   nil)]
             (swap! sample-cache assoc cache-key res)
             res))))
   :clj
   (defn reservoir-sampling
