...
(ns ^:no-doc datalevin.query.tuple
  ...
  (:require
   ...
   #?(:cljrsh nil :clj [datalevin.index :as idx])
   [datalevin.parser
    #?@(:cljrsh [:refer [BindIgnore BindScalar BindTuple]] :clj [])])
  #?(:cljrsh nil
     :clj (:import
...
(defn doc-ref->eav
  ...
  (if (and (vector? doc-ref)
           (= :g (first doc-ref)))
    (let [d #?(:cljrsh
                 (throw (ex-info "giant doc-refs are not produced by the cljrs store"
                                 {:doc-ref doc-ref}))
                 :clj (idx/gt->datom lmdb (second doc-ref)))]
                ...
                (let [giant?      (giant-doc-ref? doc-ref)
                      ...
                      datom        (when (and
                                     ...
                                     #?(:cljrsh
                                        (throw
                                          (ex-info
                                            "giant doc-refs are not produced by the cljrs store"
                                            {:doc-ref doc-ref}))
                                        :clj (idx/gt->datom
                                               lmdb (second doc-ref))))
