...
(ns ^:no-doc datalevin.query.resolve
  ...
  (:require
   ...
   [datalevin.parser :as dp
    #?@(:cljrsh [:refer [BindColl BindIgnore BindScalar BindTuple RulesVar
                         SrcVar]] :clj [])]
   [datalevin.pipe :as p]
   ...
   [datalevin.relation :as r
    #?@(:cljrsh [:refer [Relation]] :clj [])]
   [datalevin.rules :as rules]
   [datalevin.util :as u :refer [raise concatv]])
  #?(:cljrsh nil
     :clj (:import
           [java.util HashMap HashSet IdentityHashMap List]
           [datalevin.parser BindColl BindIgnore BindScalar BindTuple RulesVar
            SrcVar]
           [datalevin.relation Relation]
           [org.eclipse.collections.impl.list.mutable FastList])))

;; cljrs port note: runs near-verbatim on the runtime's mutable
;; collection shims; jlist? stands in for the java.util.List interface
;; check (Clojure sequential colls and the ArrayList/FastList shim).
#?(:cljrsh
   (defn- jlist?
     [x]
     (or (sequential? x)
         (= "java.util.ArrayList" (str (type x))))))
...
(extend-protocol IBinding
  ...
  (in->rel [binding coll]
    (cond
      ...
      (and #?(:cljrsh (jlist? coll)
              :clj (instance? java.util.List coll))
...
(defn- attach-needed-meta
  ...
  (let [v        (vec args)
    ...
    (cond
      ...
      #?(:cljrsh (or (coll? last-arg) (symbol? last-arg))
         :clj (instance? clojure.lang.IObj last-arg))
          ...
          (if (nil? val)
            ...
            (let [bound-rel
                  (cond
                    ...
                    (and projection
                         #?(:cljrsh (jlist? val)
                            :clj (instance? java.util.List val)))
