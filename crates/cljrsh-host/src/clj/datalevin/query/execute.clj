...
(ns ^:no-doc datalevin.query.execute
  ...
  (:require
   ...
   [datalevin.db :as db]
   #?@(:cljrsh [] :clj [[datalevin.index :as idx]])
   ...
   #?@(:cljrsh []
       :clj [[datalevin.query.access.fulltext :as qfulltext]
   ...
   #?@(:cljrsh [] :clj [[datalevin.spill :as sp]])
   [datalevin.timeout :as timeout]
   [datalevin.util :as u :refer [cond+ concatv map+]])
  #?(:cljrsh nil
     :clj (:import
           [java.util Comparator HashSet List PriorityQueue]
           [datalevin.parser BindTuple Constant FindColl FindRel FindScalar
            FindTuple Pattern Variable]
           [org.eclipse.collections.impl.list.mutable FastList])))

;; cljrs port note: fulltext/idoc/vector access methods gate to :clj
;; (their storage features are not ported); ordering comparators are
;; plain fns; top-k uses sort+window instead of a bounded heap; the
;; result set is a plain set (no disk spill).
#?(:cljrsh
   (require '[datalevin.parser
              :refer [BindTuple Constant FindColl FindRel FindScalar
                      FindTuple Pattern Variable]]))

(def ^:private plugin-inputs qo/plugin-inputs)

(def ^:private function-access-method
  (qfunction/access-method
    #?(:cljrsh {}
       :clj {:fulltext qfulltext/access-method
    ...
    (let [bound      (qresolve/bound-vars context)
      ...
      (when (and (= entity current-entity)
                 ...
                 (qor/exact-inequality-range?
                   #?(:cljrsh
                      (if-let [vt (:db/valueType attr-schema)]
                        (if (identical? vt :db.type/tuple)
                          (if-let [tts (attr-schema :db/tupleTypes)]
                            tts
                            (if-let [tt (attr-schema :db/tupleType)]
                              [tt] :data))
                          vt)
                        :data)
                      :clj (idx/value-type attr-schema)))
...
(defn collect
  ...
  (if (= result-set #{})
    context
    (assoc context :result-set (into #?(:cljrsh #{}
                                        :clj (sp/new-spillable-set)) (map vec)
