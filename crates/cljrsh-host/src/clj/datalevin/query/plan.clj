...
(ns ^:no-doc datalevin.query.plan
  ...
  (:require
   [clojure.set :as set]
   #?@(:cljrsh [] :clj [[datalevin.bits :as b]])
   [datalevin.constants :as c]
   [datalevin.db :as db]
   [datalevin.inline :refer [update assoc]]
   [datalevin.join :as j]
   #?@(:cljrsh [] :clj [[datalevin.lmdb :as l]])
   [datalevin.pipe :as p]
  ...
  #?(:cljrsh nil
     :clj (:import
           [java.util AbstractCollection ArrayList Collection Collections
            HashSet List]
           [java.util.concurrent Callable CompletableFuture ConcurrentHashMap
            ExecutorService Executors ForkJoinPool ForkJoinWorkerThread Future
            TimeUnit]
           [datalevin.db DB]
           [datalevin.storage Store]
           [org.eclipse.collections.impl.list.mutable FastList])))

;; cljrs port note: runs on the mutable collection shims; the portable
;; arms below replace roaring 64-bit bitmaps with sets, and writing?
;; is always true, which collapses every parallel pipeline onto the
;; upstream sequential fallback branches.
#?(:cljrsh
   (do
     (defn- bitmap64 [] (volatile! #{}))
     (defn- bitmap64-add [bm v] (vswap! bm conj (long v)) bm)
     (defn- bitmap64-cardinality [bm] (count @bm))
     (defn- bitmap64-contains? [bm v] (contains? @bm (long v)))
     (defn- bitmap64-min [bm] (reduce min @bm))
     (defn- bitmap64-max [bm] (reduce max @bm))
     (defn- bitmap64->longs [bm] (long-array (sort @bm)))
     ;; A sink whose .add calls F (interop method dispatch routes here).
     (defprotocol IFnSink
       (add [this tuple])
       (size [this]))
     (deftype FnSink [f n]
       IFnSink
       (add [_ tuple]
         (let [added? (boolean (f tuple))]
           (when added? (vswap! n inc))
           added?))
       (size [_] @n)))
   :clj
   (do
     (defn- bitmap64 [] (b/bitmap64))
     (defn- bitmap64-add [bm v] (b/bitmap64-add bm v))
     (defn- bitmap64-cardinality [bm] (b/bitmap64-cardinality bm))
     (defn- bitmap64-contains? [bm v] (b/bitmap64-contains? bm v))
     (defn- bitmap64-min [bm] (b/bitmap64-min bm))
     (defn- bitmap64-max [bm] (b/bitmap64-max bm))
     (defn- bitmap64->longs [bm] (b/bitmap64->longs bm))))
...
(defn reusable-sip-bitmap
  ...
  (when-let [future (sip-domain-future (:sip-domain-id step))]
    ...
    (let [bitmap      (.get ^CompletableFuture future)
          cardinality (bitmap64-cardinality bitmap)
...
(defn- modify-init-step-for-sip
  ...
  [init-step bm]
  (let [cardinality (bitmap64-cardinality bm)]
    (if (<= cardinality ^long c/sip-range-threshold)
      (let [values     (bitmap64->longs bm)
        ...
        (assoc init-step :range (if old-range
                                  ...
                                  new-ranges)))
      (let [min-v     (bitmap64-min bm)
            max-v     (bitmap64-max bm)
            new-range [[[:closed min-v] [:closed max-v]]]
            old-range (:range init-step)
            bm-pred   (qpred/shareable-predicate
                        (fn [v] (bitmap64-contains? bm v)))
...
(defn sip-execute-pipe
  ...
  (let [join-attr   (sip-target-attr link)
        ...
        cardinality (bitmap64-cardinality bm)]
...
(defn- writing?
  [db]
  #?(:cljrsh true
     :clj (l/writing? (.-lmdb ^Store (.-store ^DB db)))))

#?(:cljrsh
(defn- prefix-set-sink
  [^long width]
  (let [keys   (volatile! #{})
        key-of (if (= 1 (long width))
                 (fn [^objects tuple] (aget tuple 0))
                 (fn [^objects tuple]
                   (mapv #(aget tuple (int %)) (range width))))
        add-prefix!
        (fn [tuple]
          (let [k (key-of tuple)]
            (if (contains? @keys k)
              false
              (do (vswap! keys conj k) true))))
        contains-prefix?
        (fn [tuple] (contains? @keys (key-of tuple)))]
    [(->FnSink add-prefix! (volatile! 0)) contains-prefix?]))
