...
(ns ^:no-doc datalevin.query.optimizer.range
  ...
  (:require
   ...
   [clojure.walk :as w]
   #?(:cljrsh [datalevin.built-ins :as bi]
      :clj [datalevin.bits :as b])
   [datalevin.constants :as c]
   #?@(:cljrsh [[datalevin.parser :refer [Predicate SrcVar]]] :clj [])
...
(def ^:const max-string
  #?(:cljrsh (apply str (repeat 511 (char 0xFFFF)))
     :clj (b/text-ba->str c/max-bytes)))
...
(defn- wildcard-free-like-pattern
  ...
  #?(:cljrsh (bi/compile-like pattern escape false)
     :clj (LikeFSM/isValid (.getBytes pattern) (or escape \!)))
