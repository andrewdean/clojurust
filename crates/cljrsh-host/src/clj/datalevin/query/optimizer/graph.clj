...
(ns ^:no-doc datalevin.query.optimizer.graph
  ...
  (:require
   [clojure.set :as set]
   [datalevin.db :as db]
   #?(:cljrsh [datalevin.parser
               :refer [And Constant DefaultSrc Function Not Or Pattern
                       Predicate RuleExpr Variable]]
      :clj [datalevin.index :as idx])
   [datalevin.query.optimizer.range :as qor]
   [datalevin.query-util :as qu]
   [datalevin.util :as u :refer [raise]])
  #?(:cljrsh nil
     :clj (:import
           [datalevin.parser And Constant DefaultSrc Function Not Or Pattern
            Predicate RuleExpr Variable])))

;; Portable copy of datalevin.index/value-type (the index ns is
;; storage-flavored and not vendored).
#?(:cljrsh
   (defn- idx-value-type
     [props]
