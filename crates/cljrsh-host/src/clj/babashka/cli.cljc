;; cljrsh: vendored from org.babashka/cli 0.8.65 with one modification:
;; parse-number is reimplemented portably (the original uses
;; java.io.PushbackReader, unavailable without a JVM).
(ns babashka.cli
...
(defn- parse-number [x]
  #?(:cljrsh (let [v (try (edn/read-string x) (catch Exception _ ::bad))]
               (if (number? v)
                 v
                 (throw-unexpected x)))
